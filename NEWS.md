...
* Changes `truncate_feature_names` argument of `vi()` and `vip()` to `abbreviate_feature_names` which tries to abbreviate feature names to a certain length, rather than just truncating them.
