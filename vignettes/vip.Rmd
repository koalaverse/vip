...
Starting with __vip__ v0.1.3, we have included a new function `vit()` for constructing html-based variable importance tables. The primary difference between `vi()` and `vit()` is that the latter includes an `Effect` column that displays a [sparkline](https://en.wikipedia.org/wiki/Sparkline) representation of the partial dependence function for each feature. This is a concise way to display both feature importance and feature effect information in a single table. See `?vip::vit` for details. We illustrate the basic use of `vit()` in the code chunks below.

```{r pima ranger}
# First, compute a tibble of variable importance scores using any method
var_imp <- vi(rfo2, method = "permute", metric = "auc", pred_fun = pfun,
              target = "diabetes", reference_class = "neg")

# Next, convert to an html-based data table with sparklines
vit(var_imp, fit = rfo2)
...
nn %>%  # start with the previously trained neural network model
  ...
  vit()  # add sparkline representation of averaged ICE curves
