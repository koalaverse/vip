...
```{r pdp-ppr, fig.width=7, fig.asp=0.5, out.width="100%", fig.cap="PDPs of main effects in the PPR model fit to the simulated Friedman data."}
# Load required packages
library(pdp)
...
```{r ice-ppr, fig.width=7, fig.asp=0.5, out.width="100%", fig.cap="ICE curves for each feature in the PPR model fit to the simulated Friedman data. The red curve represents the PDP (i.e., the averaged ICE curves)."}
...
Starting with \pkg{vip} 0.1.4 you can now construct VI scores based on \dfn{Shapley values} REF. Suppose we make a prediction for an observation $x\prime$, denoted $f\left(x\prime\right)$. The Shapley values for $f\left(x\prime\right)$...
