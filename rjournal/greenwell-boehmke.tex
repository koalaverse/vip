...
permutation-based VI scores via the \code{mmpf::permutationImportance()}
function. Similar to the \pkg{iml} and \pkg{ingredients} implementation,
...
each feature.\footnote{A similar approach is taken in the \CRANpkg{vivo}
...
knowledge, this is not the case for \pkg{ingredients}.
...
the popular \dfn{Shapley values} \citet[sec. 5.9]{molnar-2019-iml}---a
...
level (generally referred to as \dfn{prediction explanations}. However,
...
expensive, the \CRANpkg{fastshap} package \citep{R-fastshap} exploits a
few computational tricks and also includes the option to perform
computations in parallel (see \code{?fastshap::explain} for details).
...
using the \CRANpkg{xgboost} package \citep{R-xgboost}; see Figure
\ref{vi-shap}. (\strong{Note:} specifying
...
use \dfn{accumulated local effect} (ALE) plots
...
available when using \code{method = "firm"}). Further, since the
...
the \CRANpkg{ALEPlot} \citep{R-ALEPlot} and \pkg{iml} packages.
