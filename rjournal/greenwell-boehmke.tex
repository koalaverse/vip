...
\dfn{interpretable machine learning} (IML).\footnote{Although
...
\dfn{global methods} for quantifying the importance of features in an ML
...
number of packages. The \CRANpkg{iml} package \citep{R-iml} provides the
\code{FeatureImp()} function which computes feature importance for
general prediction models using the permutation approach (discussed
later). It is written in \CRANpkg{R6} \citep{R-R6} and allows the user
...
(the default is to use \code{n\_sample = 1000}). This can help speed up
computation.
...
permutation-based method for RFs in packages \CRANpkg{party}
\citep{R-party} and \CRANpkg{ranger} \citep{R-ranger} to arbitrary
measures from the \CRANpkg{measures} package \citep{R-measures}.
Additionally, the functions in \pkg{varImp} include the option of using
...
not limited to, the following: \CRANpkg{vita} \citep{R-vita},
...
\code{getFeatureImportance()} on a \CRANpkg{gbm} model \citep{R-gbm}
...
computing VI scores. The idea is to use the leftover \dfn{out-of-bag}
(OOB) data to construct validation-set errors for each tree. Then, each
...
across all trees in the forest. Note that both methods for constructing
VI scores can be unreliable in certain situations; for example, when the
predictor variables vary in their scale of measurement or their number
of categories \citep{party2007a}, or when the predictors are highly
correlated \citep{strobl-2019-conditional}. The \pkg{varImp} package
discussed earlier provides methods to address these concerns for random
forests. Later on, we'll discuss a more general permutation method that
can be applied to any supervised learning model.
...
simulated \code{trn} data set allowing for all main effects and two-way
interactions, then use the \code{step()} function to perform backward
elimination. The resulting VIP is displayed in Figure
\ref{fig:vip-step}.
...
details). An example using the \CRANpkg{earth} package \citep{R-earth}
...
the \CRANpkg{pdp} package \citet{pdp2017}. The results are displayed in
...
we specify \code{nsim = 10} to request that each feature be permuted 10
...
\code{mmpf::permutationImportance} (version 0.0.5), and \code{vip::vi()}
(version 0.1.3.9000).
...
\pkg{ingredients}. It should be noted, however, that the implementations
in \pkg{vip} and \pkg{iml} can be parallelized. To the best of our
knowledge, this is not the case for \pkg{ingredients}. The code used to
generate these benchmarks can be found at \url{http://bit.ly/2TogXrq}.
...
to a number of fairness properties they posses
...
(say, \textgreater{}3,000 observations). The \pkg{fastshap} package
...
Starting with \pkg{vip} version 0.0.4 you can now use
\texttt{method\ =\ "shap"} (or \texttt{vip::vi\_shap()}) to compute
global Shapley-based VI scores using the method described above
(provided you have the \pkg{fastshap} package installed)---see
\texttt{?vip::vi\_shap} for details. To illustrate, we compute
Shapley-based VI scores from an \CRANpkg{xgboost} model
\citep{R-xgboost} using the Friedman data from earlier; the results are
displayed in Figure \ref{vi-shap}.\footnote{Note that the
...
produce results that are highly misleading. For example, the standard
approach to computing permutation-based VI scores involves independently
permuting individual features. This implicitly makes the assumption that
the observed features are statistically independent. In practice,
however, features are often not independent which can lead to
...
similar modification can be applied to PDPs
\citet{parr-2019-technical}\footnote{A basic R implementation is
...
\CRANpkg{plyr} \citep{R-plyr}---which relies on \CRANpkg{foreach}
...
with the \CRANpkg{doParallel} backend \citep{R-doParallel} using a
...
0.1.3.9000. In terms of future development, \pkg{vip} can be expanded in
