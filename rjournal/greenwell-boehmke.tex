...
\code{getFeatureImportance()} on a \pkg{ranger} model unless it was fit
using \pkg{mlr}.
...
\code{method = "permute"} in the call to \code{vi()} (or you can use
\texttt{vi\_permute()} directly) or \code{vip()}. Note that using
...
(version 0.1.4) was the fastest, followed closely by \pkg{ingredients}
...
\code{add\_sparklines()} in the code chunk below.
