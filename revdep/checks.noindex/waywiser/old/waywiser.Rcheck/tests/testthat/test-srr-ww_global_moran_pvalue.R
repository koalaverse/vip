...
test_that("srr: expected failures for ww_global_moran_pvalue", {
  ...
  #' Users can error:
  expect_snapshot(
    ww_global_moran_pvalue_vec(worldclim_predicted$response, worldclim_predicted$predicted, worldclim_weights),
    error = TRUE
  )
  ...
  #' @srrstats {G5.8a} Zero-length data:
  expect_snapshot(
    ww_global_moran_pvalue(head(worldclim_predicted, 0), response, predicted, structure(list(), class = "listw")),
    error = TRUE
  )
  ...
  #' @srrstats {G5.8a} Zero-length data:
  expect_snapshot(
    ww_global_moran_pvalue(head(worldclim_predicted, 0), predicted, response, structure(list(), class = "listw")),
    error = TRUE
  )
  ...
  expect_snapshot(
    ww_global_moran_pvalue_vec(NA_real_, NA_real_, structure(list(neighbours = 1), class = "listw")),
    error = TRUE
  )
  ...
  expect_snapshot(
    ww_global_moran_pvalue_vec(worldclim_simulation$response, worldclim_simulation$response, worldclim_weights)
  )
...
test_that("other generic srr standards", {
  ...
  expect_equal(
    ...
    withr::with_seed(
      123,
      ww_global_moran_pvalue(noised_worldclim, response, predicted)
    )
  )
  ...
  expect_equal(
    ...
    withr::with_seed(
      123,
      ww_global_moran_pvalue(noised_worldclim, predicted, response)
    )
  )
  ...
  expect_equal(
    ...
    withr::with_seed(
      123,
      ww_global_moran_pvalue_vec(noised_worldclim$predicted, noised_worldclim$response, noised_weights)
    )
  )
  ...
  expect_equal(
    ...
    withr::with_seed(
      123,
      ww_global_moran_pvalue_vec(noised_worldclim$response, noised_worldclim$predicted, noised_weights)
    )
  )
