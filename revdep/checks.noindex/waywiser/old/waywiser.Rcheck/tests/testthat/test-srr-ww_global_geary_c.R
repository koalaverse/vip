...
test_that("srr: expected failures for ww_global_geary_c", {
  ...
  #' Users can error:
  expect_snapshot(
    ww_global_geary_c_vec(worldclim_predicted$predicted, worldclim_predicted$response, worldclim_weights),
    error = TRUE
  )

  #' Users can error:
  expect_snapshot(
    ww_global_geary_c_vec(worldclim_predicted$response, worldclim_predicted$predicted, worldclim_weights),
    error = TRUE
  )
  ...
  #' @srrstats {G5.8a} Zero-length data:
  expect_snapshot(
    ww_global_geary_c(head(worldclim_predicted, 0), response, predicted, structure(list(), class = "listw")),
    error = TRUE
  )
  ...
  #' @srrstats {G5.8a} Zero-length data:
  expect_snapshot(
    ww_global_geary_c(head(worldclim_predicted, 0), predicted, response, structure(list(), class = "listw")),
    error = TRUE
  )
  ...
  expect_snapshot(
    ww_global_geary_c_vec(NA_real_, NA_real_, structure(list(neighbours = 1), class = "listw")),
    error = TRUE
  )
  ...
  expect_snapshot(
    ww_global_geary_c_vec(worldclim_simulation$response, worldclim_simulation$response, worldclim_weights)
  )
...
test_that("other generic srr standards", {
  ...
  expect_equal(
    ...
    withr::with_seed(
      123,
      ww_global_geary_c(noised_worldclim, response, predicted)
    )
  )
  ...
  expect_equal(
    ...
    withr::with_seed(
      123,
      ww_global_geary_c(noised_worldclim, predicted, response)
    )
  )
  ...
  expect_equal(
    ...
    withr::with_seed(
      123,
      ww_global_geary_c_vec(noised_worldclim$predicted, noised_worldclim$response, noised_weights)
    )
  )
  ...
  expect_equal(
    ...
    withr::with_seed(
      123,
      ww_global_geary_c_vec(noised_worldclim$response, noised_worldclim$predicted, noised_weights)
    )
  )
  ...
  #' @srrstats {SP6.2} Testing with ~global data
  expect_equal(
    ww_global_geary_c(guerry_modeled, predictions, Crm_prs)$.estimate,
    ww_global_geary_c(guerry_modeled_geo, predictions, Crm_prs)$.estimate
  )
  ...
  #' @srrstats {SP6.2} Testing with ~global data
  expect_equal(
    ww_global_geary_c(guerry_modeled, Crm_prs, predictions)$.estimate,
    ww_global_geary_c(guerry_modeled_geo, Crm_prs, predictions)$.estimate
  )
  ...
  #' @srrstats {SP6.2} Testing with ~global data
  expect_equal(
    ww_global_geary_c_vec(guerry_modeled$Crm_prs, guerry_modeled$predictions, guerry_weights),
    ww_global_geary_c_vec(guerry_modeled_geo$Crm_prs, guerry_modeled_geo$predictions, guerry_weights_geo)
  )
  ...
  #' @srrstats {SP6.2} Testing with ~global data
  expect_equal(
    ww_global_geary_c_vec(guerry_modeled$predictions, guerry_modeled$Crm_prs, guerry_weights),
    ww_global_geary_c_vec(guerry_modeled_geo$predictions, guerry_modeled_geo$Crm_prs, guerry_weights_geo)
  )
  ...
  expect_snapshot(
    withr::with_seed(
      123,
      ww_global_geary_c(worldclim_loaded, bio13, bio19)
    )
  )
  ...
  expect_snapshot(
    withr::with_seed(
      123,
      ww_global_geary_c_vec(worldclim_loaded$bio13, worldclim_loaded$bio19, worldclim_weights)
    )
  )
  ...
  expect_snapshot(
    withr::with_seed(
      123,
      ww_global_geary_c(worldclim_loaded, bio13, bio19, function(data) ww_build_weights(ww_make_point_neighbors(data, k = 5)))
    )
  )
